...
void main() {
	...
	if (gl_LocalInvocationIndex == 0) {
		...
		////		vec2 texCoord = (vec2(TILE_SIZE, TILE_SIZE) * tileId + vec2(x, y)) / vec2(1280, 720); // viewport size;
		////		float preDepth = texture(depthTex, texCoord).x;
