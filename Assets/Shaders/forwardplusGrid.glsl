...
//	tile[tileIndex].extent = vec4(AABBExtent(minAABB, maxAABB), 0.0);
	...
	screenSpace[3] = vec4(vec2(gl_WorkGroupID.x + 1, gl_WorkGroupID.y + 1) * TILE_SIZE, -1.0f, 1.0f);
	 ...
	 // screenSpace[0] = vec4(vec2(gl_WorkGroupID.x, gl_WorkGroupID.y + 1) * TILE_SIZE, -1.0, 1.0); // Top left
    // screenSpace[1] = vec4(vec2(gl_WorkGroupID.x + 1, gl_WorkGroupID.y + 1) * TILE_SIZE, -1.0, 1.0); // Top Right
    // screenSpace[2] = vec4(gl_WorkGroupID.xy * TILE_SIZE, -1.0, 1.0); // Bottom left
    // screenSpace[3] = vec4(vec2(gl_WorkGroupID.x + 1, gl_WorkGroupID.y) * TILE_SIZE, -1.0, 1.0); // Bottom Right
