...
void Camera::UpdateCamera(float dt) {
	...
	const Matrix4 yawRotation = Matrix4::Rotation(yaw, Vector3(0, 1, 0));

	if (Window::GetKeyboard()->KeyDown(KeyboardKeys::W)) {
		position += yawRotation * Vector3(0, 0, -1) * frameSpeed;
	}
	...
	if (Window::GetKeyboard()->KeyDown(KeyboardKeys::A)) {
		position += yawRotation * Vector3(-1, 0, 0) * frameSpeed;
	}
